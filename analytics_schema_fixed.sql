-- =====================================================
-- ESQUEMA DE ANALYTICS CORREGIDO - SIN ÍNDICES PROBLEMÁTICOS
-- =====================================================
...
BEGIN
    ...
    -- Crear tabla page_events si no existe
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'page_events') THEN
    ...
    -- Crear tabla daily_metrics si no existe
    ...
    -- Crear tabla page_metrics si no existe
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'page_metrics') THEN
    ...
    -- Crear tabla traffic_sources si no existe
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'traffic_sources') THEN
...
-- Crear índices básicos (sin funciones DATE())
...
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'user_sessions' AND policyname = 'Admin analytics read access') THEN
        CREATE POLICY "Admin analytics read access" ON user_sessions FOR SELECT USING (auth.role() = 'authenticated');
    END IF;
    
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'page_events' AND policyname = 'Admin analytics read access') THEN
        CREATE POLICY "Admin analytics read access" ON page_events FOR SELECT USING (auth.role() = 'authenticated');
    END IF;
    
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'daily_metrics' AND policyname = 'Admin analytics read access') THEN
        CREATE POLICY "Admin analytics read access" ON daily_metrics FOR SELECT USING (auth.role() = 'authenticated');
    END IF;
    
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'page_metrics' AND policyname = 'Admin analytics read access') THEN
        CREATE POLICY "Admin analytics read access" ON page_metrics FOR SELECT USING (auth.role() = 'authenticated');
    END IF;
    
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'traffic_sources' AND policyname = 'Admin analytics read access') THEN
    ...
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'user_sessions' AND policyname = 'Public analytics write access') THEN
        CREATE POLICY "Public analytics write access" ON user_sessions FOR INSERT WITH CHECK (true);
    END IF;
    
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'page_events' AND policyname = 'Public analytics write access') THEN
        CREATE POLICY "Public analytics write access" ON page_events FOR INSERT WITH CHECK (true);
    END IF;
END $$;

-- Insertar datos de ejemplo para hoy (solo si no existen)
DO $$
BEGIN
    -- Insertar sesiones de ejemplo solo si no existen
    IF NOT EXISTS (SELECT 1 FROM user_sessions WHERE session_id LIKE 'session_1_%') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM page_events WHERE session_id LIKE 'session_1_%') THEN
